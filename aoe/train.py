...
def _nli_pairs(split: str) -> List[Tuple[str, str, int]]:
	dataset = load_nli_dataset(split)
...
def _stsb_pairs(split: str) -> List[Tuple[str, str, int]]:
...
def _gis_pairs(split: str) -> List[Tuple[str, str, int]]:
...
def build_dataloader(
	...
	split: str,
	batch_size: int,
	max_length: int,  # kept for interface symmetry; currently unused
	method: str,  # currently unused but reserved for future adjustments
) -> DataLoader:
	...
	if task == "nli":
		pairs = _nli_pairs(split)
	elif task == "stsb":
		pairs = _stsb_pairs(split)
	elif task == "gis":
		pairs = _gis_pairs(split)
	else:
...
def train_epoch(
	...
	for texts1, texts2, labels in dataloader:
		...
		if method == "baseline":
			z1 = encoder.encode(texts1, device=device)
			z2 = encoder.encode(texts2, device=device)
			...
			z1_re, z1_im = encoder.encode(texts1, device=device)
			z2_re, z2_im = encoder.encode(texts2, device=device)
