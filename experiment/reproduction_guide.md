...
# 2x RTX 3090 Configuration
# Effective batch size = 64 * 2 (GPUs) * 2 (Accum) = 512
...
# First configure accelerate: `accelerate config`
BATCH_SIZE=64 \
GRAD_ACCUM_STEPS=4 \
LEARNING_RATE=4e-5 \
NLI_EPOCHS=2 \
...
**Output:**
Checkpoints will be saved to `output/bert_nli_aoe_lab1/ckpt`.
...
CKPT=output/bert_nli_aoe_lab1/ckpt \
TASKS=all \
MODEL_NAME=bert_nli_standard \
...
### 5.1 STS-B In-domain
**Training**: Fine-tune NLI model on STS-B Train.
...
**Evaluate**:
```bash
CKPT=output/bert_stsb_aoe_lab1/ckpt \
TASKS=STSBenchmark \
MODEL_NAME=bert_stsb_indomain \
bash scripts/eval_sts.sh
```

### 5.2 GIS In-domain
**Training**: Fine-tune NLI model on GIS Train.
...
CKPT=output/bert_nli_aoe_lab1/ckpt \
MODEL_NAME=bert_nli_downstream \
bash scripts/eval_downstream.sh

# Or evaluate any other checkpoint
CKPT=output/bert_stsb_aoe_lab1/ckpt \
MODEL_NAME=bert_stsb_downstream \
bash scripts/eval_downstream.sh
```
